import {
  assertFails,
  assertFalse,
  assertThrowsAsync,
  assertTrue,
  EmmettError,
  type MessageProcessor,
} from '@event-driven-io/emmett';
import {
  KurrentDBContainer,
  StartedKurrentDBContainer,
} from '@event-driven-io/emmett-testcontainers/kurrent';
import { after, afterEach, before, beforeEach, describe, it } from 'node:test';
import { v4 as uuid } from 'uuid';
import {
  kurrentDBEventStoreConsumer,
  type KurrentDBEventStoreConsumer,
} from './kurrentDBEventStoreConsumer';

void describe('KurrentDB event store consumer', () => {
  let kurrentDB: StartedKurrentDBContainer;
  let connectionString: string;
  const dummyProcessor: MessageProcessor = {
    type: 'reactor',
    id: uuid(),
    start: () => Promise.resolve('BEGINNING'),
    close: () => Promise.resolve(),
    handle: () => Promise.resolve(),
    isActive: false,
  };

  before(async () => {
    kurrentDB = await new KurrentDBContainer().start();
    connectionString = kurrentDB.getConnectionString();
  });

  after(async () => {
    try {
      if (kurrentDB) await kurrentDB.stop();
    } catch (error) {
      console.log(error);
    }
  });

  void it('creates not-started consumer for the specified connection string', () => {
    const consumer = kurrentDBEventStoreConsumer({
      connectionString,
      processors: [dummyProcessor],
    });

    assertFalse(consumer.isRunning);
  });

  void it('creates not-started consumer if connection string targets not existing KurrentDB database', () => {
    const connectionStringToNotExistingDB =
      'kurrentdb://not-existing:2113?tls=false';
    const consumer = kurrentDBEventStoreConsumer({
      connectionString: connectionStringToNotExistingDB,
      processors: [dummyProcessor],
    });

    assertFalse(consumer.isRunning);
  });

  void describe('created consumer', () => {
    let consumer: KurrentDBEventStoreConsumer;

    beforeEach(() => {
      consumer = kurrentDBEventStoreConsumer({
        connectionString,
        processors: [dummyProcessor],
      });
    });
    afterEach(() => {
      return consumer.stop();
    });

    void it('subscribes to existing event store', () => {
      consumer.start().catch(() => assertFails());

      assertTrue(consumer.isRunning);
    });

    void it('fails to start if connection string targets not existing KurrentDB database', async () => {
      const connectionStringToNotExistingDB =
        'kurrentdb://not-existing:2113?tls=false';
      const consumerToNotExistingServer = kurrentDBEventStoreConsumer({
        connectionString: connectionStringToNotExistingDB,
        processors: [dummyProcessor],
      });
      await assertThrowsAsync(
        () => consumerToNotExistingServer.start(),
        (error) => {
          return 'type' in error && error.type === 'unavailable';
        },
      );
    });

    void it('fails to start if there are no processors', async () => {
      const consumerToNotExistingServer = kurrentDBEventStoreConsumer({
        connectionString,
        processors: [],
      });
      await assertThrowsAsync<EmmettError>(
        () => consumerToNotExistingServer.start(),
        (error) => {
          return (
            error.message ===
            'Cannot start consumer without at least a single processor'
          );
        },
      );
    });

    void it(`stopping not started consumer doesn't fail`, async () => {
      await consumer.stop();

      assertFalse(consumer.isRunning);
    });

    void it(`stopping not started consumer is idempotent`, async () => {
      await consumer.stop();
      await consumer.stop();

      assertFalse(consumer.isRunning);
    });
  });

  void describe('started consumer', () => {
    let consumer: KurrentDBEventStoreConsumer;

    beforeEach(() => {
      consumer = kurrentDBEventStoreConsumer({
        connectionString,
        processors: [dummyProcessor],
      });
    });
    afterEach(() => consumer.stop());

    void it('stops started consumer', async () => {
      await consumer.stop();

      assertFalse(consumer.isRunning);
    });
  });
});
