...
import {
  KurrentDBContainer,
  type StartedKurrentDBContainer,
...
import {
  getKurrentDBEventStore,
  type KurrentDBEventStore,
} from '../kurrentDBEventStore';
import { kurrentDBEventStoreConsumer } from './kurrentDBEventStoreConsumer';

const withDeadline = { timeout: 10000 };

void describe('KurrentDB event store started consumer', () => {
  let kurrentDB: StartedKurrentDBContainer;
  let connectionString: string;
  let eventStore: KurrentDBEventStore;
  ...
  before(async () => {
    ...
    eventStore = getKurrentDBEventStore(kurrentDB.getClient());
  ...
  void describe('eachMessage', () => {
    void it(
      ...
      async () => {
        ...
        // When
        const consumer =
          kurrentDBEventStoreConsumer<ShoppingCartSummaryEvent>({
            connectionString,
    ...
    void it(
      ...
      async () => {
        ...
        const newConsumer = kurrentDBEventStoreConsumer({
          connectionString,
          processors: [inMemoryProcessor],
        });
