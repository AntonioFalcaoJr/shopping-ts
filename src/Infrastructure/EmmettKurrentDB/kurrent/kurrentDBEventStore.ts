...
export const getKurrentDBEventStore = (
  ...
  return {
    async aggregateStream<State, EventType extends Event>(
      ...
      console.log(`[DEBUG_LOG] Initial state in aggregateStream: ${JSON.stringify(state)}`);
      ...
      try {
        for await (const resolvedEvent of eventStore.readStream<EventType>(
          ...
          console.log(`[DEBUG_LOG] Applying event: ${event.type}`);
          state = evolve(state, mapFromKurrentDBEvent<EventType>(resolvedEvent));
          console.log(`[DEBUG_LOG] State after evolve: ${JSON.stringify(state)}`);
