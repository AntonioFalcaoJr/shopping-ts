...
export class ShoppingCartController {
    ...
    private setupRoutes(): void {
        ...
        this.router.patch('/carts/:cartId/items/:productId/increase', this.increaseItemQuantity.bind(this));
        this.router.patch('/carts/:cartId/items/:productId/decrease', this.decreaseItemQuantity.bind(this));
    ...
    private async increaseItemQuantity(req: Request, res: Response): Promise<void> {
        try {
            const {cartId, productId} = req.params;
            ...
            res.status(200).json({message: 'Item quantity increased'});
        } catch (error: any) {
            res.status(400).json({error: error.message});
        }
    }

    private async decreaseItemQuantity(req: Request, res: Response): Promise<void> {
        try {
            const {cartId, productId} = req.params;
            const {quantity} = req.body;
